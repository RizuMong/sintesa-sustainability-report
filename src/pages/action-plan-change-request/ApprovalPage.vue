        ...
        <MpFlex direction="column" padding="24px" gap="2">
                        ...
                        <MpTableHead>
                            <MpTableRow>
                                <MpTableCell scope="col"
                                    >Action Plan</MpTableCell
                                >
                                <MpTableCell scope="col"
                                    >Requested By</MpTableCell
                                >
                                <MpTableCell scope="col"
                                    >Requested On</MpTableCell
                                >
                                ...
                                <MpTableRow v-if="expandedId === row.id">
                                    <MpTableCell as="td" :col-span="5">
                                        ...
                                        >
                                            ...
                                            <MpFormControl
                                                :id="`change-request-notes-${row.id}`"
                                            >
                                                <MpFormLabel
                                                    >Notes / Reason</MpFormLabel
                                                >
                                                <MpText
                                                    size="label"
                                            ...
                                            <MpFormControl
                                                :id="`change-request-reviewer-notes-${row.id}`"
                                                is-required
                                            >
                                                <MpFormLabel
                                                    >Reviewer Notes</MpFormLabel
                                                >
                                                <MpTextarea
                                                    v-model="
                                                        reviewerNotes[row.id]
                                                    "
            ...
            >
                ...
                <MpText size="h3" weight="semiBold"
                    >No change requests waiting for review</MpText
                >
            </MpFlex>
        </MpFlex>
    </MpFlex>
...
const pendingItems = computed(() =>
    items.value.filter((r) => r.status === "Pending Review"),
);
...
async function reject(id: string) {
    ...
    try {
        await rejectMutation.mutateAsync({
            id,
            reviewer_notes: reviewerNotes[id]!.trim(),
        });
        toast.notify({
            id: `cr-reject-${id}`,
            variant: "success",

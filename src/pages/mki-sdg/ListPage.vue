      ...
      <template v-else-if="filteredItems.length">
        <MpTableContainer>
          <MpTable>
            <MpTableHead>
              <MpTableRow>
                <MpTableCell scope="col">Indicator Name</MpTableCell>
                <MpTableCell scope="col">Variable Code</MpTableCell>
                <MpTableCell scope="col">Input Type</MpTableCell>
                <MpTableCell scope="col">Unit</MpTableCell>
                <MpTableCell scope="col">Evidence</MpTableCell>
            ...
            <MpTableBody>
              <MpTableRow v-for="row in filteredItems" :key="row.id">
                ...
                <MpTableCell as="td" scope="row" @click="goToDetail(row)" :class="css({ cursor: 'pointer' })">
                  {{ row.evidence_attachment }}
                </MpTableCell>
                <MpTableCell as="td" scope="row" @click="goToDetail(row)" :class="css({ cursor: 'pointer' })">
                  {{ row.created_by_level === 'Subsidiary' ? (row.origin_entity?.name ?? 'Subsidiary') : 'Holding' }}
                </MpTableCell>
                <MpTableCell as="td" scope="row" @click="goToDetail(row)" :class="css({ cursor: 'pointer' })">
...
const { data, isPending } = useGetMkiSdgList()
const items = computed(() => data.value ?? [])
const isLoading = computed(() => isPending.value)

const filterColumns = computed(() => [
  { value: 'indicator_name', label: 'Indicator Name' },
  { value: 'variable_code', label: 'Variable Code' },
  {
    value: 'input_type',
    label: 'Input Type',
    options: [
...
function goToCreate() {
  router.push('/mki-sdg/detail')
}

function goToDetail(row: MkiSdg) {
  router.push({ path: '/mki-sdg/detail', query: { id: row.id } })
}
</script>
